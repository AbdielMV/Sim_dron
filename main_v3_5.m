...
% =========================================================================
fprintf('Iniciando simulación Control PID Puro...\n');

% 1. Inicialización de estado independiente
S_pid = [x(1); vx(1); y(1); vy(1); z(1); vz(1); ang(1,1); ang(2,1); ang(3,1); 0; 0; 0]; % [x; vx; y; vy; z; vz; phi; theta; psi; wx; wy; wz]
...
% Altura (Z), X y Y
kp_z = 8; kd_z = 0.5; ki_z = 5*0;
kp_x = 1.5; kd_x = 0.5; ki_x = 0.2*0;
kp_y = 1.5; kd_y = 0.5; ki_y = 0.2*0;

% Roll, Pitch, Yaw
kp_phi = 0.5; kd_phi = 0.1; ki_phi = 0.05*0;
kp_theta = 0.5; kd_theta = 0.1; ki_theta = 0.05*0;
kp_psi = 1.0; kd_psi = 0.2; ki_psi = 0.1*0;
...
for k = 1:N
    ...
    tau_dist_k = [tau_x_dist; tau_y_dist; tau_z_dist]; % Actualización del torque/disturbio para el paso actual
    ...
    ref_roll_rhonn(k+1) = ref_phi_k;
    ref_roll_rhonn(k+2) = ref_phi_k;
...
fprintf('Finalizado simulación control neuronal\n')
